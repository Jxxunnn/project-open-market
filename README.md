...
컴포넌트 단위를 더욱 작게 쪼개어 최하위 컴포넌트에서 innerHTML를 사용하고, appendChild로 요소를 상위 컴포넌트에 전달한다. 초기 설계 단계에서 재사용성을 고려하지 못했기 때문에 아토믹 패턴은 고려하지 않는다.
...
앵커 태그는 물론 게시글의 경우에도 하트 버튼에 이벤트 리스너가 추가되어야하는 상황이다. 이벤트를 추가해야하는 노드의 경우는 createElement를 사용하여 생성하였다. 고정된 HTML구조의 경우는 innerHTML로, 데이터에 따라 값이 바뀌는 경우는 createElement로 생성하는 쪽으로 결정하였다.
